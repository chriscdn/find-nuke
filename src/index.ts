...
const findNukeFile = async (
  ...
  const cond1 =
    olderThan === undefined || mtime < _olderThan.subtractFrom(now).getTime();

  // If the file has a specified extension
  const cond2 =
    extensions === undefined || extensions.includes(extension.toLowerCase());
...
const findNukeDirectory = async (
  ...
  const results = await Promise.all(
    filesInDirectory.map((fileName: string) =>
      findNukeFileOrDirectory(resolve(path, fileName), now, options),
    ),
  );
...
const doDeleteEmptyDirectories = async (
  ...
  await Promise.all(
    filesInDirectory.map(async (dirPath: string) => {
